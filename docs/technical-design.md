...
Schema evolution is numbered: the legacy baseline is version 1, thread/session normalization from #2 is version 2, scoped routing and callback claims are version 3, and bounded transcript/maintenance support is version 4. Unversioned databases run through the same idempotent baseline and transactional transformations as new databases; there is no separate ad hoc startup mutation path.
