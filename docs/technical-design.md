...
- `schema_migrations`: ordered migration version, name, and application time. Every migration runs in its own transaction; startup stops with the failing version and rollback context.
...
- `callback_tokens` and `interaction_drafts`: short chat/user-bound Telegram controls and restart-safe wizard state. Tokens carry operation, resource identity, expiry, and expected resource state; durable claim metadata prevents concurrent execution. Successful controller work commits a claim, transient failure releases it, and startup releases claims abandoned by a stopped process.
...
Schema evolution is numbered: the legacy baseline is version 1, thread/session normalization from #2 is version 2, scoped routing and callback claims are version 3, bounded transcript/maintenance support is version 4, and durable legacy tmux capture boundaries are version 5. Unversioned databases run through the same idempotent baseline and transactional transformations as new databases; there is no separate ad hoc startup mutation path.

Routine hourly maintenance removes expired or consumed callback controls, completed drafts, old sent outbox rows, stale logs and legacy tmux observations, and old resolved/orphaned actions. It preserves pending/submitting/retryable actions and any action referenced by an undelivered notification. Transcript deletion is opt-in through an explicit retention window. SQLite checkpoint support is explicit, while `/doctor` and `/health` report the schema version, database size, WAL size, and excessive-size warnings.
