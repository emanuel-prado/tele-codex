...
Codex app-server over JSON-RPC is the sole execution runtime.
...
- `/attach appserver <threadId>` attaches a specific Codex thread.
...
App-server is the only Codex runtime because it exposes structured JSON-RPC events and controls for approvals, thread resume, model selection, collaboration mode, and compaction.
...
The installer resolves `TELE_CODEX_CODEX_COMMAND` through the invoking user's `PATH`, validates that it is executable, and pins the resulting absolute path in `~/.config/systemd/user/tele-codex.service`. This keeps commands installed under locations such as `~/.local/bin` available under systemd's restricted environment. The installer enables the service immediately, restarts it after failures, and keeps Codex child processes in the same systemd control group. `service update` builds first, preserves the environment file selected during installation, revalidates and refreshes the pinned executable and unit, reloads systemd, then restarts only after those steps succeed. It verifies that the restarted unit remains active with a stable PID. If validation or the build fails, the installed unit and running service are left untouched. If lingering is disabled, follow the reported `loginctl enable-linger` instruction so the user manager survives logout and starts at boot.
...
The unattended guarantee applies to structured app-server sessions. App-server connectivity, Telegram polling, event forwarding, outbox delivery, and interaction expiry are supervised. Unexpected loop exit records a correlation ID, runs exactly-once cleanup, and exits nonzero so systemd can restart the process. High-signal notifications are persisted and delivered at least once while the host, Telegram, and Codex are reachable. Routine starts and clean or crash-loop restarts send no automatic Telegram prompt. A durable recovery card is sent only when startup invalidates a prior Active Turn or interaction; it requires explicit thread selection and is idempotent across restart and delivery retry. Threads are never resumed automatically, and `/new` remains the explicit way to open workspace controls. Secret question answers are refused because Telegram bot chats are not end-to-end encrypted.
